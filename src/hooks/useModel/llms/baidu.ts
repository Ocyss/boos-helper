...
class gpt extends llm<baiduLLMConf> {
  constructor(conf: baiduLLMConf, template: string | prompt) {
    super(conf, template);
  }
  ...
  private async post({
    ...
    const res = await request.post({
      ...
      data: JSON.stringify({
        ...
        response_format:
          this.conf.advanced.json && json ? "json_object" : "text",
      }),
      headers: {
